...
def user_add(cursor, user, host, host_all, password, encrypted, new_priv, require_ssl, check_mode):
    ...
    if password and encrypted:
        cursor.execute("CREATE USER %s@%s IDENTIFIED BY PASSWORD %s" + stmt, (user, host, password))
    elif password and not encrypted:
        cursor.execute("CREATE USER %s@%s IDENTIFIED BY %s" + stmt, (user, host, password))
    else:
        cursor.execute("CREATE USER %s@%s" + stmt, (user, host))
...
def user_mod(cursor, user, host, host_all, password, encrypted, new_priv, append_privs, require_ssl, module):
    changed = False
    msg = "User unchanged"
    grant_option = False
    ...
    for host in hostnames:
        ...
        if stmt not in current_user:
            changed = True
            msg = "Require ssl updated"
            if module.check_mode:
                return (True, msg)
            cursor.execute("ALTER USER %s@%s" + stmt, (user, host))
...
def main():
    ...
    try:
        if check_implicit_admin:
            try:
                cursor = mysql_connect(module, "root", "", config_file, ssl_cert, ssl_key, ssl_ca, db, connect_timeout=connect_timeout)
            except Exception:
                pass

        if not cursor:
            cursor = mysql_connect(module, login_user, login_password, config_file, ssl_cert, ssl_key, ssl_ca, db, connect_timeout=connect_timeout)
        ...
        if user_exists(cursor, user, host, host_all):
            try:
                if update_password == "always":
                    changed, msg = user_mod(cursor, user, host, host_all, password, encrypted, priv, append_privs, require_ssl, module)
                else:
                    changed, msg = user_mod(cursor, user, host, host_all, None, encrypted, priv, append_privs, require_ssl, module)
